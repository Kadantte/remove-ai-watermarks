...
# ///
"""Extra high Chroma1 rungs for the Meta Content Seal cohort.

The first calibration ladder topped out at 0.08 and the worst fixture
(gen_lighthouse_watercolor) was still detected there, so this appends 0.10,
0.12 and 0.15 for every Meta fixture without touching the already-oracled
lower rungs.
"""
...
app = modal.App("chroma-meta-high-rungs")
...
def run(payload: dict[str, bytes]) -> dict:
    ...
    for name, data in payload.items():
        ...
        for strength in RUNGS:
            ...
            out[f"{Path(name).stem}/chroma_s{strength:.3f}"] = {
                "png": buffer.getvalue(),
...
def main(out: str = "out/cohort-calibration/meta") -> None:
    payload = {name: (_ROOT / "data" / "contentseal" / "originals" / name).read_bytes() for name in META_FIXTURES}
